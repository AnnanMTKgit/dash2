...
section[data-testid="stSidebar"] {
    ...
    color: #212121;             /* Texte sombre pour bon contraste */
    border-right: 2px solid #BBDEFB; /* Légère bordure pour délimiter */
}

/* ========== TITRES (h1, h2, h3...) ========== */
h1, h2, h3, h4, h5 {
    color: #1976D2;  /* Bleu moderne pour les titres */
    font-weight: bold;
}

/* ========== LIENS ========== */
a, a:hover {
    color: #FF9800;  /* Orange vif pour attirer l'attention */
}
...
/* ========== BOUTONS PRINCIPAUX ========== */
button[kind="primary"] {
    background-color: #1976D2;  /* Bleu principal */
    color: white;
    border-radius: 6px;
    font-weight: bold;
}
...
button[kind="primary"]:hover {
    background-color: #1565C0; /* Bleu un peu plus foncé au survol */
}
...
/* Options individuelles du radio */
div[data-baseweb="radio"] label {
    color: #212121 !important; /* Texte sombre */
    padding: 6px 12px;
    border-radius: 4px;
    margin-right: 8px;
    font-weight: 500;
...
div[data-baseweb="radio"] input:checked + div {
    background-color: #1976D2 !important; /* Fond bleu sélectionné */
...
""", unsafe_allow_html=True)
    ...
    # Sidebar page page configuration
...
""", unsafe_allow_html=True)
    ...
    def option1(df_all,df_queue,df_RH):
        ...
        if option == page1:
            _,df=AgenceTable(df_all,df_queue)
            TMO=df["Temps Moyen d'Operation (MIN)"].mean().astype(int) 
            TMA=df["Temps Moyen d'Attente (MIN)"].mean().astype(int)
            NMC=df['Total Tickets'].mean().astype(int)
    ...
    if len(df)==0:
        ...
        if profil in ['Caissier','Clientele']:
            ...
            # if st.sidebar.button("Déconnexion",key="submit1"):
            #     # Clear session state and rerun the app to go back to login page
            #     st.session_state.logged_in = False
            #     st.session_state.username = None
            #     st.experimental_rerun()
            ...
            # --- Add Copyright to the bottom of the sidebar ---
            # You'll use st.markdown with unsafe_allow_html=True
            # The key is to add CSS to position it at the bottom.
            

            # -- À placer après avoir ajouté tout le contenu de la sidebar --
            st.sidebar.markdown("""
    <style>
    ...
    /* Ajouter une zone fixe en bas de la sidebar */
    .sidebar-footer {
        position: fixed;
        left: 0;
        bottom: 0;
        width: 17rem; /* Largeur de la sidebar par défaut */
        padding: 10px 0;
        text-align: center;
        font-size: 0.9rem;
        color: #2E2E2E;
    ...
    /* Adapter à différents thèmes ou fonds */
